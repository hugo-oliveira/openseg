...
#     net = nn.DataParallel(net)
    ...
    # Setting datasets.
    train_set = list_dataset.ListDataset(dataset_name, 'Train', (args['h_size'], args['w_size']), 'statistical', hidden, overlap=False, use_dsm=True)
    train_loader = DataLoader(train_set, batch_size=args['batch_size'], num_workers=args['num_workers'], shuffle=True)

    test_set = list_dataset.ListDataset(dataset_name, 'Test', (args['h_size'], args['w_size']), 'statistical', hidden, overlap=True, use_dsm=True)
    test_loader = DataLoader(test_set, batch_size=1, num_workers=args['num_workers'], shuffle=False)

    # Setting criterion.
    criterion = CrossEntropyLoss2d(weight=weights, size_average=False, ignore_index=args['num_classes']).cuda(args['device'])
