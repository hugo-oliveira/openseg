...
class ListDataset(data.Dataset):
    ...
    def __init__(self, dataset, mode, crop_size, normalization='minmax', hidden_classes=None):
    ...
    def make_dataset(self):
        ...
        items = []
        for it in data_list:
            item = (os.path.join(img_dir, it.replace('_noBoundary.tif', '.tif')), os.path.join(msk_dir, it))
            items.append(item)
    ...
    def test_crops(self, img, msk, msk_true):
        ...
        w_img = util.view_as_blocks(img, (self.crop_size[0], self.crop_size[1], 3)).squeeze()
    ...
    def shift_labels(self, msk):
        ...
        for h_c in self.hidden_classes:
            ...
            for c in range(h_c - cont + 1, num_classes):
                msk[msk == c] = c - 1
                msk_true[msk_true == c] = c - 1
            cont = cont + 1
        
        msk_true[msk == 100] = num_classes - len(self.hidden_classes)
        msk[msk == 100] = num_classes
